...
pub(crate) fn run_7z(tool: &Path, args: &[&std::ffi::OsStr], action: &str, cwd: Option<&Path>) -> Result<std::process::Output, String> {
...
fn extract_archive_via_7z(archive: &Path, staging: &Path, max_uncompressed_size: u64) -> Result<String, InstallError> {
    ...
    if kind.eq_ignore_ascii_case("gzip") {
        if let Some(inner) = fs::read_dir(staging).install_err()?.filter_map(|e| e.ok()).map(|e| e.path()).next() {
            run_7z(&tool, &[OsStr::new("x"), inner.as_os_str(), OsStr::new(&out_arg), OsStr::new("-y"), OsStr::new("-bd")], "extract", None).install_err()?;
...
fn download_release_asset(
    ...
    latest_installable: impl FnOnce(&str, Option<&str>) -> Result<(Value, Value), InstallError>,
    most_recent_release: impl FnOnce(&str, Option<&str>) -> Result<Value, InstallError>,
    ...
    let (release, asset) = match (overrides.release, overrides.asset) {
        ...
        (Some(r), None) => {
            let asset = pick_release_asset(r)?;
            (r.clone(), asset)
        }
        (None, Some(a)) => {
            let release = most_recent_release(repo, token)?;
            (release, a.clone())
        }
        (None, None) => latest_installable(repo, token)?,
    };
...
pub fn install_port(
    ...
    if port.source_type == SourceType::None {
        return Err(InstallError::Message("SourceType::None should never reach install_port (internal bug).".to_string()));
    }
    ...
    let archive_path = match port.source_type {
        SourceType::Github => {
            ...
            let (path, tag) = download_release_asset(
                ...
                tmp_path,
                &mut on_progress,
                |r, t| github_api::latest_installable_release(r, t, preferred_asset.as_deref()).map_err(InstallError::from),
                |r, t| github_api::most_recent_release(r, t).map_err(InstallError::from),
        ...
        SourceType::Gitlab => {
            ...
            let (path, tag) = download_release_asset(
                ...
                tmp_path,
                &mut on_progress,
                |r, t| gitlab_api::latest_installable_release(r, t, preferred_asset.as_deref()).map_err(InstallError::from),
                |r, t| gitlab_api::most_recent_release(r, t).map_err(InstallError::from),
        ...
        SourceType::None => unreachable!("SourceType::None exclu avant d'appeler install_port"),
    };
