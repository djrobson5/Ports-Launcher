...
pub(crate) fn open_info_dialog(app: &Rc<AppState>, router: &Rc<RefCell<GamepadRouter>>, port: &Port) {
    ...
    let save_path: Option<std::path::PathBuf> = game_folder.as_deref().and_then(|dir| {
        let resolved = port.save.as_ref().and_then(|v| crate::core::platform_resolve::resolve_save_folder(v, dir))?;
        crate::core::path_safety::is_within(&resolved, dir).then_some(resolved)
    });
    let save_ok = save_path.as_ref().map(|p| p.exists()).unwrap_or(false);
    dialog.set_save_folder_enabled(save_ok);

    let save2_path: Option<std::path::PathBuf> = game_folder.as_deref().and_then(|dir| {
        let resolved = port.save2.as_ref().and_then(|v| crate::core::platform_resolve::resolve_save_folder(v, dir))?;
        crate::core::path_safety::is_within(&resolved, dir).then_some(resolved)
    });
