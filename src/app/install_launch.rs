...
pub(crate) fn start_install(
    ...
    std::thread::spawn(move || {
        ...
        let paths = crate::core::installer::InstallPaths { library_dir: &library_dir, saves_backup_dir: &saves_backup_dir };
...
pub(crate) fn open_version_picker(app: &Rc<AppState>, router: &Rc<RefCell<GamepadRouter>>, port: Port) {
    ...
    std::thread::spawn(move || {
        let result = match source_type {
            ...
            SourceType::DirectUrl | SourceType::None => Err("This source has no version history.".to_string()),
        };
